...
<div class="d-flex align-items-end">
    <form action="{{ route('subsidiary_piutang') }}" method="GET" class="row g-3 align-items-end flex-grow-1">
        <div class="col-md-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function populateInvoiceDropdown(storeName, accountCode) {
            fetch(`{{ route('subsidiaryPiutang.details') }}?store_name=${encodeURIComponent(storeName)}&account_code=${encodeURIComponent(accountCode)}`)
                .then(response => {
