...
<div class="mt-2">
    <form action="{{ route('subsidiary_utang') }}" method="GET" class="row g-3 align-items-end flex-grow-1 mb-3">
        <div class="col-md-3">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function populateInvoiceDropdown(storeName, accountCode) {
            fetch(
                    `{{ route('subsidiaryUtang.details') }}?store_name=${encodeURIComponent(storeName)}&account_code=${encodeURIComponent(accountCode)}`
                    )
                .then(response => {
