...
class CumulativeGeneralLedgerExport implements FromCollection, WithColumnWidths, WithStyles, WithEvents
{
    ...
    public function __construct($voucherDetails, $month, $year, $sheetTitle = 'Cumulative General Ledger')
    {
    ...
    public function collection()
    {
        ...
        // Sort each group by voucher_date to ensure chronological order
        foreach ($groupedDetails as $mainAccountCode => $details) {
            $details = $details->sortBy(function ($detail) {
                return Carbon::parse($detail->voucher->voucher_date)->timestamp;
            });
        ...
        $period = 'Januari - ' . date('F', mktime(0, 0, 0, $this->month, 10)) . ' ' . $this->year;
        array_unshift($data, [$this->sheetTitle . ' - ' . $period], ['', '', '', '', '', '', '', '']);
